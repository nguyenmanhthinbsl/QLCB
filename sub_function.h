	...
	while ((input = getch()) != ENTER)
	{
		...
		else
		{
			if (strlen(str) == length)
				continue;
			if (type == ONLY_NUMBER && (input < '0' || input > '9'))
				continue;
			if (type == ONLY_WORD && ((input < 'A' || input > 'Z') && (input < 'a' || input > 'z') && input != ' '))
				continue;
			if (type == NO_SPACE && input == ' ')
				continue;
			if (input == ' ' && str[strlen(str) - 1] == ' ')
				continue;
			if (input >= 'a' && input <= 'z')
				input = input - 32;
			cout << input;
...
void input_date(char *date, char &key, int type)
{
	...
	while (1)
	{
		...
		{
			if ((type == FULL_DATETIME && strlen(date) == 16) || (type == ONLY_DATE && strlen(date) == 10))
				continue;
