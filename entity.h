...
int timkiem_MB(List_MayBay l, char *sohieuMB)
{
    for (int i = 0; i < l.soluong; i++)
        if (strcmp(l.nodes[i]->sohieu_maybay, sohieuMB) == 0)
            return i;
    return -1;
}
...
void Add_HK_to_List(PTR_HK &root, HanhKhach &hk)
{
    ...
    {
        if (ChangeCharToNum(hk.CMND) < ChangeCharToNum(root->info.CMND))
            Add_HK_to_List(root->left, hk);
        else if (ChangeCharToNum(hk.CMND) > ChangeCharToNum(root->info.CMND))
            Add_HK_to_List(root->right, hk);
    }
}
...
PTR_HK timkiem_HK(PTR_HK root, const char *CMND)
{
    ...
    {
        if (ChangeCharToNum(CMND) > ChangeCharToNum(nodeRun->info.CMND))
...
void sort_CB(PTR_ChuyenBay First)
{ // Selection
    ...
    for (i = First; i->next != NULL; i = i->next)
    {
        ...
        for (j = i->next; j != NULL; j = j->next)
            if (sosanhTG(min->data.ngaykhoihanh, j->data.ngaykhoihanh) > 0)
                min = j;
        // swap data
        temp = min->data;
        min->data = i->data;
